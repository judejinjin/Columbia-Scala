...
class Aquarium (val width: Int, val height: Int){
     ...
     */
    def attemptMove(element : AquariumElement) {
     ...
     */
    def handleCollision(element1 : AquariumElement, e2 : AquariumElement) {
        element1 match {
            case e1: Eating =>  
...
class Crab(locX: Int, locY: Int) extends LifeForm(locX, locY) with Moving with Eating {
    override val symbol = 'C'
}
object Crab{ def apply(locX : Int, locY : Int)  = new Crab(locX, locY) }
...
object AquariumSimulator {   
    ...
    def main(args: Array[String]) {
        ...
        aquarium.add(HungryFish(6, 6,10))
        aquarium.add(Crab(8,8))
