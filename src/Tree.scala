...
abstract class Tree[T] { 
    ...
    def leafs : List[T] =  Nil
    
    def evaluate : Int = content.toString.toInt
}
...
class Node[T](val content: T, val left: Tree[T], val right: Tree[T]) extends Tree[T]{
    ...
    def traverse[A](proc_node : (A,A,T) =>A, proc_leaf: T=>A) = 
            proc_node(left.traverse(proc_node, proc_leaf), 
                     ...
                     content)
    override def toString = "(" + content.toString + " " + left.toString + " " + right.toString + ")" 

    override def leafs :List[T]= left.leafs ::: right.leafs
...
class Leaf[T](val content: T) extends Tree[T] {
    ...
    override def leafs :List[T]= content :: Nil
